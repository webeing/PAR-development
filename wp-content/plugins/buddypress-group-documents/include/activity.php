...
function bp_group_documents_record_add( $document ) {
	...
	$params = array('content'=>sprintf( __ ('%s uploaded the file %s to %s','bp-group-documents'),bp_core_get_userlink($bp->loggedin_user->id),'<a href="' . BP_GROUP_DOCUMENTS_FILE_URL . $document->file . '">' . attribute_escape( $document->name ) . '</a>','<a href="' . bp_get_group_permalink( $bp->groups->current_group ) . '">' . attribute_escape( $bp->groups->current_group->name ) . '</a>'),
...
function bp_group_documents_record_edit( $document ) {
	...
	$params = array('content'=>sprintf( __ ('%s edited the file %s in %s','bp-group-documents'),bp_core_get_userlink($bp->loggedin_user->id),'<a href="' . BP_GROUP_DOCUMENTS_FILE_URL . $document->file . '">' . attribute_escape( $document->name ) . '</a>','<a href="' . bp_get_group_permalink( $bp->groups->current_group ) . '">' . attribute_escape( $bp->groups->current_group->name ) . '</a>'),
...
function bp_group_documents_record_delete( $document ) {
	...
	$params = array('content'=>sprintf( __ ('%s deleted the file %s from %s','bp-group-documents'),bp_core_get_userlink($bp->loggedin_user->id),$document->name,'<a href="' . bp_get_group_permalink( $bp->groups->current_group ) . '">' . attribute_escape( $bp->groups->current_group->name ) . '</a>'),
